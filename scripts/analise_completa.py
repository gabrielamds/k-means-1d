...
def load_results():
    """Carrega resultados do Windows e Colab - lê arquivos reais se existirem"""
    import os
    
    # Verificar se os arquivos de resultados reais existem
    windows_file = 'results/resultados_windows.csv'
    colab_file = 'results/resultados_colab.csv'
    
    if os.path.exists(windows_file) and os.path.exists(colab_file):
        print("   ✓ Carregando resultados reais dos arquivos CSV...")
        windows = pd.read_csv(windows_file)
        colab = pd.read_csv(colab_file)
    else:
        print("   ⚠️  Arquivos de resultados não encontrados. Gerando dados de exemplo...")
        print("   Execute os benchmarks no Colab para gerar resultados reais!")
        
        # Dados fallback (exemplo)
...
grande,MPI+CUDA,1p,419.8,3245639.673338"""
        ...
        windows = pd.read_csv(StringIO(windows_data))
        colab = pd.read_csv(StringIO(colab_data))
